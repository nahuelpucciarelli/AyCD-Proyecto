...
\subsubsection{Diseño del Lazo Externo (Control P de Posición)}
Con el lazo interno de velocidad estabilizado, el sistema visto desde la posición se aproxima a un integrador ideal, al igual que en el caso del izaje:
\begin{equation}
    \frac{X_t(s)}{V_t^\ast(s)} \approx \frac{1}{s}
\end{equation}
Se utiliza un controlador proporcional:
\begin{equation}
    v_t^\ast(t) = K_{pos} \,\bigl(x_t^\ast(t) - x_t(t)\bigr)
\end{equation}
lo que genera una dinámica de primer orden con polo en $-K_{pos}$. La ganancia $K_{pos}$ se ajusta para cumplir los límites de velocidad y aceleración del carro seleccionando un factor de separación $N$.
