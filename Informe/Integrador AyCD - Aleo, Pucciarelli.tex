...
Portacontenedores de Muelle tipo Pórtico} \\ \Large{Proyecto Global Integrador} \\ \texttt{VERSIÓN FINAL}}
...
\begin{equation}
    K_f = \frac{i_t}{r_{td}}
\end{equation}
ya que el torque en el tambor cumple $T_{td}(t)=i_t\,T_{tm}(t)$ y la fuerza lineal equivalente sobre el carro resulta $F_{cmd}(t)=T_{td}(t)/r_{td}$.
...
\begin{equation}
    \ddot{\theta}_l(t) = -\frac{g}{l_h(t)}\sin(\theta_l(t)) - \frac{\ddot{x}_t(t)}{l_h(t)}\cos(\theta_l(t))
...
\begin{equation}
    \ddot{x}_t(t) \approx \dot{v}_{ref}(t) \approx \dot{v}_{traj}(t) + \frac{d}{dt}(K_{sway} \, \theta_l(t)) = \dot{v}_{traj}(t) + K_{sway} \, \dot{\theta}_l(t)
\end{equation}
...
\begin{equation}
    \ddot{\theta}_l(t) + \left( \frac{K_{sway}}{l_h(t)} \cos(\theta_l(t)) \right) \dot{\theta}_l(t) + \frac{g}{l_h(t)}\sin(\theta_l(t)) = -\frac{\dot{v}_{traj}(t)}{l_h(t)}\cos(\theta_l(t))
...
\begin{equation}
    \ddot{\theta}_l(t) + \left( \frac{K_{sway}}{l_h(t)} \right) \dot{\theta}_l(t) + \left( \frac{g}{l_h(t)} \right) \theta_l(t) = 0
\end{equation}
...
\begin{equation}
    \frac{K_{sway}}{l_h(t)} = 2 \, \zeta \, \omega_n \implies K_{sway} = 2 \, \zeta \, l_h(t) \, \sqrt{\frac{g}{l_h(t)}}
\end{equation}
Simplificando, obtenemos la ganancia para un amortiguamiento crítico ($\zeta = 1$):
\begin{equation}
    K_{sway}(l_h) = 2 \,\sqrt{g \, l_h(t)}
\end{equation}
...
$$e_{\theta}(t) = \theta_{enc}(t) - \hat{\theta}(t)$$
...
$$\dot{\hat{\theta}}(t) = \hat{\omega}(t) + L_1 \, e_{\theta}(t)$$
$$\dot{\hat{\omega}}(t) = \frac{1}{J_{eq}} \left( T_{ref}(t) - b_{eq} \, \hat{\omega}(t) + L_3 \int e_{\theta}(t) \, dt \right) + L_2 \, e_{\theta}(t)$$
Donde:
\begin{itemize}
...
$$\hat{v}(t) = \hat{\omega}(t) \cdot k_{out}$$
...
Una vez llevado a bloques de función, es relativamente directo realizar la traducción a ST, manteniendo la misma lógica de control.
\begin{figure}[H]
    \centering
...
\subsection{Comunicación UDP Simulink/CODESYS}
Debido al retardo que se produce en la comunicación, se considera que OPC no es un protocolo adecuado para la comunicación entre la simulación y el controlador. Para probar esta hipótesis, se implementó una comunicación alternativa utilizando el protocolo UDP, que es un protocolo de comunicación sin conexión y de baja latencia. Esto permite a la simulación correr en tiempo real, sin experimentar retrasos significativos en la comunicación. Se implementó para el control supervisor y de seguridad, dejando de lado el control regulatorio, ya que requiere una mayor velocidad de comunicación que incluso con UDP es difícil de mantener.

La implementación UDP es un poco más compleja, requiere el desarrollo de un protocolo de empaquetado y desempaquetado de datos, así como la sincronización entre los sistemas. Se creó por lo tanto dos programas adicionales en CODESYS, uno para enviar los datos a Simulink (\texttt{COM\_UDP\_TX}), y otro para recibir las variables desde Simulink (\texttt{COM\_UDP\_RX}). 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{com_udp_codesys.png}
    \caption{Nuevas funciones de comunicación UDP en CODESYS.}
\end{figure}

Los archivos de trama son de tipo DUT (Data Unit Type), y se definen como estructuras que organizan los datos a enviar. El orden es vital, ya que por UDP solo se envian bytes en un vector, y el receptor debe conocer el orden y tipo de cada dato para poder desempaquetarlo correctamente. 

Los programas \texttt{COM} se encargan de repartir los valores de las variables a enviar o recibir a sus respectivas posiciones en la trama, y luego utilizan librerías con varibales de envío o recepción.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{com_udp_rx.png}
    \caption{Función de Recepción UDP en CODESYS.}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{com_udp_tx.png}
    \caption{Función de Transmisión UDP en CODESYS.}
\end{figure}

En Simulink, al igual que con la comunicación con la animación, se implementaron bloques de lectura/escritura de UDP, permitiendo enviar y recibir datos en tiempo real sin los retrasos de OPC.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{simulink_udp_codesys.png}
    \caption{Diseño UDP en Simulink.}
\end{figure}

Para lograr una sincronización adecuada, se debe programar una tarea en CODESYS que ejecute la funcion de lectura de datos más rápido que la de envío, para evitar pérdida de información. En este caso, se programó a $10$ ms, mientras que el bloque de escritura se ejecuta a $20$ ms. Además, el orden de ejecución también es vital, por lo que la prioridad más elevada es para la lectura (luego el control de seguridad, el control supervisor, y por último el envío de datos). En Simulink es necesario activar \textit{pacing} a tiempo real, es decir, 1 segundo.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{task_config.png}
    \caption{Task Configuration en CODESYS.}
\end{figure}

A continuación se muestra una captura de la simulación funcionando a través de UDP:
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{ejemplo_UDP.png}
    \caption{Simulación funcionando a través de UDP.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{codigo_auto1.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{codigo_auto2.png}
...
El desarrollo de la interfaz gráfica en Unity aportó una representación visual en tiempo real muy útil para validar el comportamiento global de la grúa. Esta etapa permitió además incorporar y comprender la comunicación UDP entre Simulink y Unity, incluyendo el empaquetado y tratamiento de datos. 

Como último comentario, esta misma comunicación UDP se pudo implementar con éxito entre Simulink y CODESYS, permitiendo un rendimiento superior de la simulación. Consideramos que podría ser una alternativa viable a OPC para este proyecto, aunque con sus propias complejidades.
...
Además, se ha preparado un video demostrativo que muestra el funcionamiento de la grúa en diferentes modos de operación, destacando la interacción entre los niveles de control y la visualización en Unity. El video se puede acceder a través del siguiente \href{https://www.youtube.com/watch?v=KdlOn0zeDz8}{enlace a Youtube}.
...
\bibitem{codesys_help} \href{https://www.helpme-codesys.com/}{CODESYS Help - Documentación oficial de CODESYS.}
\end{thebibliography}
