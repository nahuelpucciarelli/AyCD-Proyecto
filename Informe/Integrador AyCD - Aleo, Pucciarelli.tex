...
Para la implementación del sistema de control de oscilación de la carga, se ha optado por una estrategia de Control en Cascada Desacoplado, en lugar de un control centralizado basado en el modelo dinámico completo del sistema (Lagrangiano). A continuación, se fundamenta esta decisión y se deduce la ley de control utilizada.
...
\begin{enumerate}
    \item \textbf{Dinámica del Accionamiento (Lazo Interno):} El control de velocidad del motor del carro tiene una respuesta rápida (tiempo de establecimiento $< 100$ ms).
    \item \textbf{Dinámica del Péndulo (Lazo Externo):} La oscilación de la carga es un fenómeno lento, con periodos naturales en el orden de los 2 a 12 segundos (dependiendo de la altura de izaje $l_h$).
\end{enumerate}

Bajo la hipótesis de que el lazo interno está correctamente sintonizado, se asume que la velocidad real del carro $v_t$ sigue a la referencia de velocidad $v_{ref}$ con una dinámica despreciable:
\begin{equation}
...
En esta arquitectura, la referencia de velocidad total enviada al variador se compone de la velocidad de trayectoria planificada más la acción de control antibalanceo: $v_{ref} = v_{traj} + v_{sway}$. 
Si definimos una ley de control antibalanceo estrictamente proporcional al ángulo:
\begin{equation}
    v_{sway} = K_{sway} \, \theta_l
    \label{eq:ley_control}
\end{equation}
La aceleración resultante del carro impuesta al péndulo será la derivada de la referencia total:
\begin{equation}
    \ddot{x}_t \approx \dot{v}_{traj} + \frac{d}{dt}(K_{sway} \, \theta_l) = \dot{v}_{traj} + K_{sway} \, \dot{\theta}_l
\end{equation}
Sustituyendo esta expresión en la ecuación (\ref{eq:pendulo}) y reordenando los términos, se obtiene la ecuación diferencial del sistema en lazo cerrado:
\begin{equation}
    \ddot{\theta}_l + \left( \frac{K_{sway}}{l_h} \right) \dot{\theta}_l + \left( \frac{g}{l_h} \right) \theta_l = -\frac{1}{l_h}\dot{v}_{traj}
    \label{eq:lazo_cerrado}
\end{equation}
Este resultado es fundamental: al inyectar una señal de velocidad proporcional al ángulo, la cinemática del sistema transforma esa señal en una aceleración proporcional a la velocidad angular. Esto introduce el término de amortiguamiento viscoso en el péndulo (actuando dinámicamente como un compensador PD), mientras que la aceleración de la trayectoria $\dot{v}_{traj}$ actúa únicamente como una fuerza forzante que excita el sistema de manera transitoria.

\subsubsection{Sintonía y Gain Scheduling (LPV Simplificado)}
Para garantizar un desempeño uniforme frente a la variación de la longitud del cable, se comparan los coeficientes de la ecuación (\ref{eq:lazo_cerrado}) con la ecuación canónica de un sistema de segundo orden ($s^2 + 2\zeta\omega_n s + \omega_n^2 = 0$), donde $\omega_n = \sqrt{g/l_h}$ (frecuencia natural del péndulo). Igualando el término de amortiguamiento:
\begin{equation}
...
\end{equation}
Dado que la altura de izaje determina directamente la frecuencia natural del péndulo, el modelo se interpreta como un sistema de parámetros variables (LPV). Para mantener una respuesta invariante, se ajusta la ganancia $K_{sway}$ dinámicamente en función de $l_h$.

A esta ganancia teórica se le debe aplicar un factor de sintonía ($\lambda$) para ajustar el nivel de amortiguamiento (equivalente a cambiar $\zeta$). Esto se debe a que la velocidad del carro es limitada, por lo que es necesaria una ganancia menor para evitar sobrecorrecciones en la posición de la carga. Después de varias pruebas, se seleccionó $\lambda = 0.45$ para lograr un compromiso entre amortiguamiento efectivo y estabilidad robusta.

\subsubsection{Factor de Corrección}
Si bien la derivación teórica sugiere una ganancia dependiente únicamente de la longitud del cable, la implementación práctica enfrenta limitaciones por saturación de actuadores. Con carga máxima, la asunción de respuesta instantánea del carro se degrada, introduciendo retardos que pueden desestabilizar el lazo si la ganancia es muy agresiva.

Para resolver este compromiso, se modula la ganancia base mediante un factor de compensación de inercia ($\lambda_m$):
\begin{equation}
    K_{sway}(l_h, m_{est}) = K_{teorico}(l_h) \, \lambda_{m}(m_{est})
\end{equation}
La ganancia disminuye cuando la carga ($m_{est}$) es elevada, priorizando la estabilidad del actuador frente a la rapidez de amortiguamiento. Físicamente, esta acción de control provoca que el carro acelere ``debajo'' de la carga cuando esta se adelanta, y frene cuando se atrasa, disipando la energía cinética del péndulo de manera óptima sin desestabilizar el lazo de velocidad interno.

\subsubsection{Conclusiones}
La arquitectura resultante corresponde a un esquema de control jerárquico ampliamente utilizado en grúas industriales. El controlador de trayectoria genera la referencia principal de movimiento, mientras que el compensador de balanceo actúa en paralelo modulándola. Esta estrategia permite controlar un sistema subactuado introduciendo amortiguamiento efectivo, sin requerir de un complejo modelo Lagrangiano acoplado.
