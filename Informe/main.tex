...
Para el control del carro, se opta por una arquitectura de control en cascada (Posición $\to$ Velocidad $\to$ Torque).

\subsubsection{Modelo del Carro para Control de Velocidad}
Partiendo de la ecuación de accionamiento del carro \eqref{accionamiento_carro}, y agrupando las perturbaciones mecánicas en un torque equivalente, se tiene:
\begin{equation}
    J_{eq,t}\,\frac{d \, \omega_{tm}(t)}{dt} + b_{eq,t}\,\omega_{tm}(t) = T_{tm}(t) - T_{dist}(t)
\end{equation}
donde
\begin{equation}
    T_{dist}(t) = \underbrace{\frac{r_{td}}{i_t}\,F_{tw}(t)}_{\text{Carga en el cable}}
    - \underbrace{T_{tb}(t,\textcolor{cyan}{BRK_t})}_{\text{Freno}}
    + \Delta_{model}(t)
\end{equation}
Usando la relación cinemática $v_t(t) = \frac{r_{td}}{i_t}\,\omega_{tm}(t)$, la función de transferencia velocidad--torque queda:
\begin{equation}
    G_{vel,t}(s) = \frac{V_t(s)}{T_{tm}(s)} = \frac{K_t}{J_{eq,t}\,s + b_{eq,t}}, \quad K_t = \frac{r_{td}}{i_t}
\end{equation}
El polo mecánico resulta $P_{mec} = -b_{eq,t}/J_{eq,t}$.
...
\end{equation}
Con lazo unitario, el denominador del lazo cerrado es:
\begin{equation}
    J_{eq,t}\,s^2 + \left(b_{eq,t} + K_p K_t\right)s + K_i K_t
\end{equation}
...
\begin{equation}
    K_p = \frac{2\,\zeta\,\omega_n\,J_{eq,t} - b_{eq,t}}{K_t}, \quad
    K_i = \frac{\omega_n^2\,J_{eq,t}}{K_t}
\end{equation}
...
\subsubsection{Diseño del Lazo Externo (Control P de Oosición)}
Con el lazo interno de velocidad estabilizado, el sistema visto desde la posición se aproxima a un integrador ideal:
\begin{equation}
    \frac{X_t(s)}{V_t^\ast(s)} \approx \frac{1}{s}
\end{equation}
Se utiliza un controlador proporcional:
\begin{equation}
    v_t^\ast(t) = K_x \,\bigl(x_t^\ast(t) - x_t(t)\bigr)
\end{equation}
lo que genera una dinámica de primer orden con polo en $-K_x$. La ganancia $K_x$ se ajusta para cumplir los límites de velocidad y aceleración del carro.
...
\end{equation}
Se selecciona $\omega_n \approx 8$ rad/s para asegurar rapidez sin excitar la resonancia del cable, y $\zeta = 0.707$ o $1$ para evitar sobrepasos. Esta sintonía garantiza que el sistema tenga la ``rigidez'' necesaria para sostener la carga, independientemente de la baja fricción mecánica.
...
\subsubsection{Saturación de Velocidad (Límite Operativo):}
...
\begin{itemize}
    \item $P_{h,nom}$: Potencia nominal disponible para izaje.
    \item $v_{h,max}$: Velocidad máxima mecánica.
    \item $M_{base}$: Carga límite para operación a velocidad máxima.
\end{itemize}
...
\end{equation}
A esta ganancia teórica se le puede aplicar un factor de sintonía ($\lambda$) para ajustar el nivel de amortiguamiento deseado. Debido a que la velocidad del carro es limitada, es posible que se requiera una ganancia menor a la teórica para evitar sobrecorrecciones en la posición de la carga.

\subsubsection{Sintonía Robusta y Factor de Corrección}
Si bien la derivación teórica sugiere una ganancia dependiente únicamente de la longitud del cable, la implementación práctica enfrenta una limitación no lineal que el modelo simplificado ignora:

\textbf{Saturación de Actuadores (Inercia Variable):} Con carga máxima, el motor opera cerca de sus límites de torque. La asunción de respuesta instantánea del carro ($v_t \approx v_{ref}$) se degrada, introduciendo retardos que pueden desestabilizar el sistema si la ganancia es muy agresiva.

Para resolver este compromiso, se implementa una estrategia que modula la ganancia base mediante un factor de corrección dinámico:
\begin{equation}
    K_{sway}(l_h, m_{est}, \theta_l) = K_{teorico}(l_h) \, \underbrace{\lambda_{m}(m_{est})}_{\text{Inercia}}
\end{equation}
Donde el factor de sintonía se define como \textbf{compensación de inercia ($\lambda_{m}$):} Disminuye la ganancia cuando la carga es elevada para evitar correcciones bruscas debido a la inercia del carro.

Finalmente, la señal $ v_{sway}$ se suma a la referencia de velocidad proveniente del lazo de posición. Físicamente, esto provoca que el carro acelere ``debajo'' de la carga cuando esta se adelanta, y frene cuando se atrasa, disipando la energía cinética del péndulo de manera óptima independientemente de la altura de izaje y de la masa. 

\subsection{Observadores de Estado}
Se puede notar de las ecuaciones desarrolladas que la realimentación de velocidad es un parámetro crítico para garantizar la estabilidad de los lazos internos. Dado que la planta física cuenta únicamente con mediciones de posición a través de encoders, la obtención de la velocidad mediante una derivada directa amplifica drásticamente el ruido de cuantificación de los sensores, desestabilizando la señal de control.

Para solucionar este problema y obtener una estimación limpia de la velocidad ($\hat{\omega}$), se implementaron observadores de estado extendidos. Estos bloques combinan un modelo físico-matemático del sistema con un lazo de corrección basado en el error de estimación de posición.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{control_carro.png}
    \caption{Control Regulatorio del Carro (Trolley).}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{control_izaje.png}
    \caption{Control Regulatorio del Izaje (Hoist).}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{control_regulatorio.png}
...
\begin{itemize}
    ...
    \item \textbf{Modo Automático:} Es el estado más complejo, ya que debe controlar la grúa sin señales del operador. Incluye múltiples estados internos que permiten seleccionar el contenedor a mover. Luego toma el control de la grúa para mover el mismo desde un punto A hasta un punto B. A traves de dos funciones Matlab, genera una curva de movimiento segura, moviendo el spreader a estados intermedios que salen como consignas de posición al control regulatorio, el cual las suaviza mediante la Curva S para cumplir las limitaciones de velocidad y aceleración. Estas funciones manejan dos casos distintos: uno cuando no se conoce el perfil de obstáculos, el cual genera una trayectoria segura, y otro cuando sí se conoce, utilizando el mapa de alturas generado por el bloque paralelo de \textit{Índices y Perfil}.
    \item \textbf{Modos de Error y Posición Segura:} En caso de detectarse una falla (propia del control supervisor o del control de seguridad), la máquina aborta cualquier operación actual y fuerza al sistema a adoptar una posición de resguardo para proteger la integridad de la carga y el equipo. La recuperación se puede activar con un botón de reset, pero esto es principalmente para pruebas, ya que en un entorno real se requeriría una inspección física antes de reanudar la operación.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{maquina_estados.png}
...
\subsection{Manejo de Entradas del Operador}
Se encarga del pre-procesamiento de las señales provenientes del operador. Escala, filtra y traduce los comandos crudos del hardware (como la deflexión de un joystick) en consignas cinemáticas limpias (referencias de velocidad) que el control regulatorio pueda interpretar correctamente. Se divide en dos bloques paralelos: uno se encarga exclusivamente de la gestion de las inputs del joystick, limitando movimientos en ciertas situaciones (como la sobrecarga) y suavizando las consignas de velocidad para que no superen los límites de seguridad, el otro bloque se encarga de actualizar la denominada ``cruceta'', que es la interfaz que muestra al operador dónde está el spreader en tiempo real, permitiendo que se tenga una referencia visual de la posición de la carga (fundamental para realizar movimientos precisos). 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.66\textwidth]{manejo_cruceta.png}
    \caption{Manejo de Cruceta en Stateflow.}
\end{figure}

\subsection{Análisis Espacial y Perfil de Obstáculos}
Para que el modo automático pueda trazar rutas seguras, la grúa debe conocer su entorno. Este bloque procesa los datos de los sensores de escaneo del LIDAR y actualiza continuamente un mapa de alturas o perfil de obstáculos bajo el área de trabajo. Esto garantiza que cualquier trayectoria generada esquive las pilas de contenedores existentes. Al mismo tiempo, calcula los índices en los que se encuentra el carro. El perfil de alturas se representa como una envolvente de seguridad, con margenes laterales y superiores que garantizan que la grúa no intente pasar por zonas donde pueda colisionar con obstáculos.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{indices_perfil.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{control_supervisor.png}
...
Este hilo supervisa constantemente el estado físico de la máquina a través de la redundancia de sensores. Durante la operación normal, el sistema permanece en el estado \texttt{SISTEMA\_OK}. 
...
\begin{itemize}
    \item \textbf{Emergencia de Izaje:} Se activa si la velocidad observada supera en un $15\%$ al límite dinámico calculado (sobrevelocidad), o si se activan los finales de carrera superior/inferior (\texttt{FCE\_sup}, \texttt{FCE\_inf}).
    \item \textbf{Emergencia de Carro:} Se activa si el carro impacta contra los finales de carrera extremos (\texttt{FCE\_izq}, \texttt{FCE\_der}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{estados_seguridad.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{watchdog.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{control_seguridad.png}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.4\textwidth}
        \centering
        \includegraphics[width=\linewidth]{unity1.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.4\textwidth}
        \centering
        \includegraphics[width=\linewidth]{unity2.png}
    \end{subfigure}
...
\subsection{Generación de Código de StateFlow a texto estructurado (ST)}
COMPLETAAAAAR

\section{Codesys}

Finalmente, para la implementación en el hardware real, se tradujo la lógica desarrollada en Simulink a un programa en Codesys utilizando el lenguaje de programación estructurado (ST). Este proceso implicó mapear cada bloque funcional a funciones y estructuras de control equivalentes en Codesys, asegurando que la lógica de control se mantuviera intacta durante la transición al entorno de ejecución real.

Puede verse a continuación la estructura resultante:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{arbol_codesys.png}
    \caption{Codesys - Árbol de componentes.}
\end{figure}

Los 3 POUs principales corresponden a los 3 niveles de control:
\texttt{Control\_Regulatorio}, \texttt{Control\_Supervisor} y \texttt{Control\_Seguridad}.

En la realidad, estas 3 funciones se ejecutan de forma cíclica y en paralelo, sin embargo en Codesys, debido a la utilización de un PLC virtual con una sola CPU, estas tareas se ejecutan de forma secuencial pero de manera intercalada; sin embargo se ejecuta a tal velocidad que, a fines prácticos, el comportamiento idéntico al de un sistema con múltiples CPUs ejecutando tareas en paralelo.

\subsection{Control Regulatorio (Nivel 2) en Codesys}

Remitiéndose a la sección de diseño del control regulatorio, se rediseñó el control en Matlab usando la misma lógica pero representándola en bloques de función de Matlab:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{control_carro_funciones.png}
    \caption{Control de carro pero como funciones de Matlab.}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{control_izaje_funciones.png}
    \caption{Control de izaje pero como funciones de Matlab.}
\end{figure}

Una vez llevado a bloques de función, fue fácil realizar la traducción a código estructurado en Codesys, manteniendo la misma lógica de control y estructura en cascada.

\subsection{Control Supervisor (Nivel 1) en Codesys}

El control supervisor es un conjunto de varios procesos en paralelo:
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{codesys_supervisor1.png}
    \caption{Codesys - Diagrama general de control supervisor.}
\end{figure}
Notar que difiere del diseñado en Simulink en que se agrega un estado \texttt{LECTURA\_BOTONES}, el cuál se encarga exclusivamente de detectar flancos de subida de los botones presionados por el usuario y guardar dichos valores en variables. Esto no es necesario en Matlab ya que StateFlow cuenta con funciones de detección de flancos de subida y bajada.

Se puede ver a continuación la máquina de estados encargada de alternar entre los modos de Stand By, Manual, Automático, Error y Posición Segura. Esta máquina de estados es el hilo de ejecución principal del supervisor, el cual se encarga de coordinar la operación general de la grúa.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{codesys_supervisor2.png}
    \caption{Codesys - Supervisor - Máquina de estados.}
\end{figure}

En la siguiente imágen puede verse la implementación utilizada para el módo automático, la cual replica la ya implementada en Codesys.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{codesys_supervisor3.png}
    \caption{Codesys - Supervisor - Máquina de estados - Auto.}
\end{figure}
Como detalle, cabe mencionar que en Codesys, al salir del modo Auto, y volver a entrar, éste se sigue ejecutando desde donde quedó. Es por eso que se tuvo que implementar un flag que detecta si ya se había empezado a ejecutar el modo Auto previamente y acciona una transición para reiniciar la máquina de estados interna del modo Auto, asegurando que cada vez que se entre a este modo, se ejecute la lógica desde el estado inicial. De esta manera se evita comportamientos erráticos o inconsistentes al alternar entre modos. Esto no ocurre en Stateflow, ya que al entrar el modo Auto, éste comienza a ejecutarse desde el estado inicial por defecto.

Se replica además la máquina de estados encargada de la lógica de los Twistlocks, encargada de detectar si es posible cerrar o abrir los twistlocks y de hacer una estimación de la masa de la carga, la cuál se utiliza para limitar la velocidad de izaje y detectar sobrecarga.
\begin{figure}[H]
    \centering
...
Además es necesario poder procesar las entradas analógicas provenientes de las palancas accionadas por el operario para traducirlas a consignas de velocidad:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{codesys_supervisor5.png}
    \caption{Codesys - Supervisor - Manejo de Cruceta (inputs analógicos de joystick).}
\end{figure}

De manera similar, se utilizan las entradas de los sensores para calcular la posición discreta del carro y actualizar el perfil de obstáculos, información que es fundamental para la generación de trayectorias seguras en modo automático:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{codesys_supervisor6.png}
    \caption{Codesys - Supervisor - Índices Perfil.}
\end{figure}

\subsection{Control Seguridad (Nivel 0) en Codesys}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{codesys_seguridad2.png}
    \caption{Codesys - Supervisor - Máquina de estados.}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Codesys - Supervisor - Watchdog.}
\end{figure}

\subsection{Comuniación Simulink-Codesys}
Para poder utilizar el PLC virtual de Codesys con el programa cargado en el mismo, se utilizó el protocolo de comunicación industrial \texttt{OPCUA}. La versión de Codesys utilizada (CODESYS V3.5 SP21 Patch 2) cuenta con la posibilidad de montar un servidor OPCUA y de seleccionar qué variables del programa se quieren exponer a través de este protocolo. Por otro lado, Matlab R2025b cuenta con un cliente OPCUA que permite conectarse a este servidor y leer/escribir las variables expuestas en tiempo real.

Se detectó que al hacer lectura y escritura desde el cliente de OPCUA en Simulink con el tiempo de muestreo de 0.02 s, retrazaba en gran medida la ejecución de la simulación. Para solucionar este problema, se decidió implementar bloques de comunicación que se ejecuten a diferentes tiempos de muestreo:
\begin{table}[H]
    \centering
    \caption{Tiempos de muestreo de comunicación OPCUA y variables asociadas.}
    \label{tab:ts_opcua}
    \begin{tabular}{p{0.25\textwidth} p{0.68\textwidth}}
        \toprule
        \textbf{Tiempo de muestreo} & \textbf{Variables/funciones que operan con ese tiempo} \\
        \midrule
        $T_s = 20$ ms & \begin{itemize}
            \item Heartbeat de Watchdog.
            \item Input de palancas del operador.
        \end{itemize} \\
        \addlinespace
        $T_s = 500$ ms & \begin{itemize}
            \item Resto de variables del Sistema Supervisor.
            \item Resto de variables del Sistema de Seguridad.
        \end{itemize} \\
        \addlinespace
        $T_s = 100$ ms & \begin{itemize}
            \item Variables del Controlador Carro.
            \item Variables del Controlador Izaje.
        \end{itemize} \\
        \bottomrule
    \end{tabular}
\end{table}

Luego de realizadas pruebas, se ha notado una mejora significativa en la fluidez de la simulación, manteniendo una comunicación efectiva entre Simulink y Codesys. Y se ha podido comprobar de forma satisfactoria que el sistema responde de manera exactamente igual a como lo hace utilizando la variante de StateFlow.
