...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{control_carro_funciones.png}
    \caption{Control del Carro con funciones de Matlab.}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{control_izaje_funciones.png}
...
La máquina de estados se desarrolla de forma simular, y se encarga de alternar entre los modos de Stand By, Manual, Automático, Error y Posición Segura. 
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{codesys_supervisor3a.png}
...
Para el manejo de la cruceta y las palancas de los joysticks, se utilizó un SFC paralelo, comportandose igual al modelo de Stateflow: 
\begin{figure}[H]
    \centering
...
Los bloques de \textit{Límites Suave} y \textit{Watchdog Heartbeat} se mantienen prácticamente iguales, desarrollados en ST directamente debido a su sencillez.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{config_OPC.png}
\end{figure}
...
El desarrollo de este proyecto demostró la viabilidad y robustez de aplicar la metodología de Diseño Basado en Modelos para la automatización de maquinaria pesada. La integración escalonada desde la física pura hasta el código de control industrial permite extraer las siguientes conclusiones fundamentales:

\subsection{Validación Virtual y Diseño de Control}
El uso del entorno MATLAB/Simulink probó ser una herramienta invaluable como laboratorio de ensayo virtual. Modelar la física no lineal de la grúa —incluyendo la cinemática del péndulo, inercias variables y moduladores de torque electromagnéticos— permitió validar algoritmos de control complejos sin poner en riesgo hardware físico real. La sintonización analítica de los controladores regulatorios y de los observadores de estado garantizó un desempeño óptimo y determinista.

\subsection{Transición a la Arquitectura Industrial}
La migración de los modelos teóricos continuos al entorno de programación IEC 61131-3 en CODESYS evidenció el salto cualitativo entre la matemática ideal y las restricciones del hardware industrial. Se concluye que la estructuración del código requiere un manejo estricto del determinismo temporal:
\begin{itemize}
    \item \textbf{Gestión de Tareas (Tasking):} Fue imperativo separar la ejecución en diferentes hilos jerárquicos. El control regulatorio exige tiempos de escáner ultrarrápidos ($T_{s2} = 1$ ms) para mantener la estabilidad numérica de la integración discreta, mientras que las tareas lógicas operan eficientemente a menores frecuencias ($T_{s1} = 20$ ms).
    \item \textbf{Estructuración del Código:} La combinación de Texto Estructurado (ST) para la resolución de ecuaciones diferenciales y Diagramas de Funciones Secuenciales (SFC) para las máquinas de estado supervisoras, resultó en una arquitectura de software escalable.
\end{itemize}

\subsection{Co-Simulación}
La implementación de una animación en Unity, comunicado mediante Simulink vía OPC UA, validó la lógica de Nivel 1 (Control Supervisor) y Nivel 0 (Seguridad). Una conclusión crítica de esta etapa fue la necesidad de desacoplar las frecuencias de control y comunicación. Intentar transmitir datos por red a la velocidad del lazo interno asfixia los recursos del procesador y satura la red. La solución de implementar bloques de comunicación con tiempos de muestreo más lentos permitió mantener la integridad de la simulación sin comprometer la seguridad, aunque introdujo un retardo que debe ser considerado en un entorno real.

En síntesis, este flujo de trabajo garantiza que la lógica de control, las medidas de seguridad ante fallas y la gestión de trayectorias que gobiernan el modelo virtual son estructuralmente idénticas a las que se cargarían en el Controlador Lógico Programable (PLC) de una grúa real, reduciendo drásticamente los tiempos de puesta en marcha y asegurando los más altos estándares de seguridad operativa.
