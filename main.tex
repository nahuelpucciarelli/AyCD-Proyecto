...
\begin{equation}
    N_{x_t} = floor\left( \frac{x_{t_{max}} - x_{t_{min}}}{W_c} \right)+1
\end{equation}

Teniendo en cuenta que $x_{t_{min}}=-30\,m$ (sobre el muello) y $x_{t_{max}}=50\,m$ (sobre el barco), y que $W_c=2.44\,m$ (ancho estándar de contenedor ISO 20'), se obtiene que $N_{x_t} = 33$. Los posibles índices serán entonces:
\begin{equation}
    i = 1, 2, 3, \ldots, 33
\end{equation}
Luego, la última posición discreta corresponde a:
\begin{equation}
    x_{33} = -30\,m + (33-1) \cdot 2.44\,m = 48.08\,m
\end{equation}
...
\begin{equation}
    i = floor\left( \frac{(x_t(t)+3\,m) - x_{t_{min}}}{W_c} \right)
\end{equation}
