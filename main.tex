...
\end{equation}
En este trabajo se adopta $\zeta_t = 0.707$ y $\omega_{n,t} = 3$ rad/s para un compromiso entre rapidez y sobrepaso.
...
\end{equation}
con un limitador de aceleracion opcional para suavizar la consigna. Para valvular $K_{p,x}$, se elige el lazo externo varias veces mas lento que el lazo interno de velocidad (tipicamente 3--10$\times$), por lo que se selecciona:
\begin{equation}
...
\subsubsection{Optimización del Lazo de Posición}

En el control del carro, existe un compromiso fundamental entre la rapidez de respuesta (productividad) y la capacidad de amortiguar el balanceo (estabilidad).
\begin{itemize}
    \item Una ganancia proporcional alta ($K_{pos}$ grande) reduce el tiempo de viaje, pero vuelve al sistema ``rígido'', impidiendo que el controlador Anti-Sway realice las pequeñas correcciones necesarias al llegar al destino.
    \item Una ganancia baja permite un excelente control del balanceo, pero hace que la aproximación final sea excesivamente lenta.
\end{itemize}

Para resolver este conflicto, se implementa una ganancia $K_{pos}$ no lineal variable en función del error de posición ($e_{pos}$):
\begin{equation}
    K_{pos}\,e_{pos} = 
    \begin{cases} 
        K_{crucero} & \text{si } |e_{pos}| > D_{umbral} \quad (\text{Lejos: alta agresividad}) \\
        K_{llegada} & \text{si } |e_{pos}| \le D_{umbral} \quad (\text{Cerca: alta suavidad}) 
    \end{cases}
\end{equation}
Cuando el error es grande, se utiliza una ganancia alta ($K_{crucero} \approx 3.0$) para minimizar el error de arrastre y seguir fielmente el perfil de velocidad trapezoidal. Al acercarse al objetivo (ej. $< 5$ m), la ganancia disminuye suavemente ($K_{llegada} \approx 0.5$). Esto reduce la ``rigidez'' del lazo de posición, otorgando mayor autoridad al controlador Anti-Sway para realizar maniobras de retroceso o avance fino, necesarias para detener la carga con ángulo residual nulo.

Esta estrategia híbrida permite cumplir simultáneamente con los requisitos de tiempo de ciclo corto y precisión en la estabilización de la carga.
