...
Dado que el sistema de izaje posee límites estrictos de seguridad —velocidad máxima variable según la carga y aceleración acotada para evitar daños estructurales— no es viable ingresar referencias de tipo escalón al controlador.

Se introduce un bloque **Generador de Trayectoria (Curva-S)** que procesa la consigna de posición del supervisor y genera perfiles cinemáticos suaves en tiempo real. Este módulo asegura que:
\begin{enumerate}
    \item La velocidad de referencia nunca supere el límite dinámico calculado por la curva de potencia: $|v_{ref}| \le v_{lim}(\hat{m}_l)$.
    \item La aceleración se mantenga dentro del rango seguro ($|a_{ref}| \le 0.75 \, \text{m/s}^2$).
    \item Se minimice el \textit{Jerk} (cambio brusco de aceleración) para evitar la excitación de oscilaciones en el cable.
\end{enumerate}

El generador entrega la curva de posición ideal ($y_{ref}$) al lazo de control.
