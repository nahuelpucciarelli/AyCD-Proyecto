...
Para organizar esta lógica compleja, la arquitectura se dividió en múltiples procesos paralelos, permitiendo que la máquina evalúe simultáneamente el modo de operación, la seguridad y el entorno.

\subsection{Máquina de Estados}
Es el hilo de ejecución primario que dicta el comportamiento global de la grúa. Gestiona de manera segura las transiciones entre los distintos modos de operación:
\begin{itemize}
    \item \textbf{Stand By:} Estado de reposo y espera de comandos.
    \item \textbf{Modo Manual:} Permite al operador tomar el control directo de las velocidades de los ejes a través del joystick.
    \item \textbf{Modo Automático:} Toma el control de la grúa para mover un contenedor desde un punto A hasta un punto B. Internamente, este estado cuenta con una sub-máquina algorítmica encargada de planificar una trayectoria libre de colisiones y ejecutarla por etapas.
    \item \textbf{Modos de Error y Posición Segura:} En caso de detectar una falla, la máquina aborta la operación actual y fuerza a los ejes a adoptar una posición de resguardo para proteger la integridad de la carga y el equipo.
\end{itemize}
\begin{figure}[H]
    \centering
...
Se encarga del pre-procesamiento de las señales provenientes del operador. Escala, filtra y traduce los comandos crudos del hardware (como la deflexión de un joystick) en consignas cinemáticas limpias (referencias de velocidad o posición) que el control regulatorio pueda interpretar correctamente.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{manejo_cruceta.png}
    \caption{Manejo de Cruceta en Stateflow.}
\end{figure}

\subsection{Análisis Espacial y Perfil de Obstáculos}
Para que el modo automático pueda trazar rutas seguras, la grúa debe "ver" su entorno. Este bloque procesa los datos de los sensores de escaneo del LIDAR y actualiza continuamente un mapa de alturas o perfil de obstáculos bajo el área de trabajo. Esto garantiza que cualquier trayectoria generada esquive las pilas de contenedores existentes.
\begin{figure}[H]
    \centering
...
Constituye un anillo de seguridad de software vital. Este bloque emite y espera recibir pulsos de vida (\textit{heartbeats}) a intervalos regulares desde la estación de operación remota. Si el contador de pulsos perdidos supera un límite preestablecido, el sistema asume una pérdida de comunicación, interrumpe cualquier movimiento en curso y envía a la máquina principal al estado de error.
\begin{figure}[H]
    \centering
...
Dentro de este estado nominal, el bloque realiza un cálculo dinámico crítico: ajusta en tiempo real el límite de velocidad de izaje permitido ($vh\_lim$) en función de la masa actual de la carga. Esto garantiza que el motor nunca exceda su potencia nominal ($P_{nom}$), comportándose según una curva de potencia constante:
$$vh\_lim = \min \left( vh\_max\_unloaded, \max \left( vh\_nom, \frac{P_{nom}}{(m_{container} + M_s) g} \right) \right)$$

El sistema evaluará continuamente condiciones de falla para transicionar a estados de emergencia específicos, en los cuales se aplican inmediatamente los frenos de operación y emergencia ($BRK_{t} = 0$, $BRK_{hE} = 0$):
\begin{itemize}
    ...
    \item \textbf{Emergencia Manual:} Detención forzada accionada por el operador mediante el botón físico de golpe de puño (\texttt{BTN\_Emerg}).
    \item \textbf{Emergencia de Comunicación (\texttt{EMERGENCIA\_WD}):} Disparada por la pérdida de enlace con la estación de control.
\end{itemize}

Una vez mitigada la causa de la parada, el sistema exige un rearme manual (\texttt{BTN\_Reset}) que hace transicionar a la máquina a un \texttt{MODO\_RECUPERACION} antes de permitirle volver a operar normalmente.
\begin{figure}[H]
    \centering
...
Este segundo proceso paralelo funciona como un supervisor de la integridad de la red de datos. La estación de control remota (control supervisor) debe enviar una señal de latido (\textit{heartbeat}) que alterna su estado constantemente. 

La máquina de estados del Watchdog alterna entre \texttt{HEARTBEAT\_DOWN} y \texttt{HEARTBEAT\_UP} persiguiendo esta señal. Si la señal se congela y el sistema permanece en un solo estado por una cantidad de ciclos de reloj que supere el límite seguro (\texttt{counter\_limit}), se declara una falla de red (\texttt{FALLA\_WD = 1}), lo que dispara la parada de emergencia en el proceso principal para evitar que la grúa continúe moviéndose con el último comando recibido.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{watchdog.png}
...
\section{Comunicación con Unity}
