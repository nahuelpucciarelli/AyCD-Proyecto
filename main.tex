...
\subsection{Control PID de Traslación del Carro}
...
\begin{equation}
    G_t(s) = \frac{V_t(s)}{T_{tm}(s)} = \frac{K_f}{M_t\,s + b_t}, \quad K_f = \frac{i_t}{r_{td}}
\end{equation}
...
\subsubsection{Sintonía PID por asignación de polos}
Se define el error $e_v(t) = v_t^\ast(t) - v_t(t)$ y la ley de control:
\begin{equation}
    T_{tm}^*(t) = K_p\,e_v(t) + K_i\,\int e_v(t)\,dt + K_d\,\frac{d e_v(t)}{dt}
\end{equation}
siendo $T_{tm}^*(t)$ la consigna saliente del controlador y que es dirigida hacia el actuador. Con realimentación unitaria, el polinomio característico resulta:
\begin{equation}
    s\,(M_t\,s + b_t) + K_f\,\left(K_d\,s^2 + K_p\,s + K_i\right) = 0
\end{equation}
equivalente a:
\begin{equation}
    (M_t + K_f\,K_d)\,s^2 + (b_t + K_f\,K_p)\,s + K_f\,K_i = 0
\end{equation}
Se desea ubicar los polos del lazo cerrado a una década de rapidez respecto del polo mecánico, por lo que se adopta un polinomio deseado:
\begin{equation}
    \Delta_d(s) = (s + a)^2, \quad a = 10\,\frac{b_t}{M_t}
\end{equation}
Igualando coeficientes se obtiene:
\begin{equation}
    K_p = \frac{2\,a\,(M_t + K_f\,K_d) - b_t}{K_f}, \quad
    K_i = \frac{a^2\,(M_t + K_f\,K_d)}{K_f}
\end{equation}
La ganancia derivativa $K_d$ permite ajustar la inercia efectiva y el compromiso ruido--rapidez; una elección conservadora es $K_d = 0$, que conduce a un PI con:
\begin{equation}
    K_p = \frac{2\,a\,M_t - b_t}{K_f}, \quad
    K_i = \frac{a^2\,M_t}{K_f}
\end{equation}

\subsubsection{Generacion de consigna de velocidad desde posicion}
...
\begin{equation}
    v_t^\ast(t) = \mathrm{sat}_{v_{t,\max}}\left(K_x\,\left(x_t^\ast(t) - x_t(t)\right)\right)
\end{equation}
con un limitador de aceleracion opcional para suavizar la consigna. Para valvular $K_x$, se elige el lazo externo varias veces mas lento que el lazo interno de velocidad (tipicamente 3--10$\times$). Por ejemplo, si el lazo interno se ajusto con $a = 10\,b_t/M_t$, puede tomarse $K_x \approx a/3$ o $a/10$ y luego ajustar segun saturaciones y limites de aceleracion.

\subsection{Control alternativo del carro con estructura en cascada}
Para mantener coherencia con el esquema usado en el control de izaje, se adopta una arquitectura en cascada (Posición $\to$ Velocidad $\to$ Torque) y se diseña el lazo interno sobre la velocidad $v_t$, usando $T_{tm}$ como entrada de actuador.
...
\subsubsection{Compensación de fricción (Feedforward)}
Para reducir el esfuerzo integral, se puede sumar una pre-alimentación viscosa:
\begin{equation}
    T_{ff}(k) = b_{eq,t}\,\omega_{tm}^\ast(k) \quad \Rightarrow \quad
    T_{tm}^\ast(k) = T_{PI}(k) + T_{ff}(k)
\end{equation}
Si se dispone de un modelo de fricción Coulomb, puede añadirse un término $\pm T_c$ según el sentido de marcha.
...
\subsubsection{Sintonía Robusta y Factores de Corrección}
Si bien la derivación teórica sugiere una ganancia dependiente únicamente de la longitud del cable, la implementación práctica enfrenta dos limitaciones no lineales que el modelo simplificado ignora:
\begin{enumerate}
    \item \textbf{Saturación de Actuadores (Inercia Variable):} Con carga máxima, el motor opera cerca de sus límites de torque. La asunción de respuesta instantánea del carro ($v_t \approx v_{ref}$) se degrada, introduciendo retardos que pueden desestabilizar el sistema si la ganancia es muy agresiva.
    \item \textbf{Restricciones de Seguridad:} Las especificaciones imponen límites estrictos de oscilación ($\pm 20^\circ$ en transitorios, $\pm 1^\circ$ en reposo). Un controlador lineal optimizado para la precisión en reposo puede resultar insuficiente para frenar oscilaciones grandes durante aceleraciones bruscas.
\end{enumerate}

Para resolver este compromiso, se implementa una estrategia que modula la ganancia base mediante factores de corrección dinámicos:
\begin{equation}
    K_{sway}(l_h, m_{est}, \theta_l) = K_{teorico}(l_h) \, \underbrace{\lambda_{m}(m_{est})}_{\text{Inercia}} \, \underbrace{\lambda_{\theta}(\theta)}_{\text{Seguridad}}
\end{equation}
Donde los factores de sintonía se definen como:
\begin{itemize}
    \item \textbf{Compensación de Inercia ($\lambda_{m}$):} Amplifica la ganancia cuando la carga es elevada para evitar angulos elevados debido a la inercia del carro.
    \item \textbf{Factor de Seguridad ($\lambda_{\theta}$):} Aumenta la agresividad del control cuando el ángulo de oscilación se acerca a los límites permitidos ($\pm 20^\circ$), priorizando la seguridad sobre la suavidad.
\end{itemize}
